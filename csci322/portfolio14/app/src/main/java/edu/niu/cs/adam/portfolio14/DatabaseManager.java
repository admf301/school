...
public class DatabaseManager extends SQLiteOpenHelper {
    ...
    public ArrayList<Product> selectAll() {
        ...
        while(cursor.moveToNext()) {
            ...
            Integer currentQty = cursor.getInt(2);
            Double currentPrice = cursor.getDouble(3);

            Product product = new Product(currentID, currentName, currentQty, currentPrice);
    ...
    public void updateById(int id, String name, int quantity, double price) {
        String sqlUpdate = "update " + TABLE_NAME + " set " +
                           ITEM_NAME + " = '" + name + "', " +
                           ITEM_QUANTITY + " = '" + quantity + "', " +
                           ITEM_PRICE + " = '" + price + "'" + " where " +
                           ITEM_ID + " = " + id;
